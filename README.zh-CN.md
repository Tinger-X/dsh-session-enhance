...
| | 功能 | 你能得到什么 |
|---|---|---|
| 🗂️ | **归档管理** | 侧栏会话菜单一键归档；**设置 → 归档管理** 统一管理：搜索、按时间/标题排序、按工作区筛选、取消归档、按项目/全部恢复、确认后批量删除。 |
...
在 **设置 → 归档管理** 管理已归档会话：搜索、排序、按工作区筛选、**同步记录**、恢复与删除。
...
2. **管理** — 设置 → **归档管理**：搜索、排序（更新时间/创建时间/字母序）、按项目筛选、恢复、删除（一律确认）。
...
7. **复制会话 ID** — 侧栏会话菜单 → *复制ID*，把该对话的 session id 复制到剪贴板（已归档会话同样可用）。
