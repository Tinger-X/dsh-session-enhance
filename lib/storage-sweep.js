...
/**
 * dsh-session-manager 物理删除的磁盘清扫层。
 *
...
async function sweepStorageFile(file, sessionId, logger) {
	...
	try {
		...
		logger.warn(`dsh-session-manager: could not read storage file "${file}" for sweep: ${String(error)}`);
		return false;
	}
	let unit;
	try {
		...
		// 这里只告警，留给下次删除或人工修复。
		logger.warn(`dsh-session-manager: storage file "${file}" is not valid JSON, skipping direct sweep: ${String(error)}`);
		return false;
	}
	...
	try {
		...
		logger.info(`dsh-session-manager: physically removed all traces of "${sessionId}" from ${file}`);
		return true;
	} catch (error) {
		await rm(tmp, { force: true }).catch(() => {});
		logger.warn(`dsh-session-manager: could not rewrite storage file "${file}" (traces of "${sessionId}" remain on disk): ${String(error)}`);
		return false;
	}
}
/**
...
async function sweepStorageRoot(root, sessionId, logger) {
	...
	try {
		...
		logger.warn(`dsh-session-manager: storages root "${root}" is not enumerable, skipping direct sweep: ${String(error)}`);
		return [];
	}
	const rewritten = [];
