...
var SessionEnhanceWorkspaceRegistry = class extends WorkspaceRegistry {
	...
	constructor(ctx) {
		...
		markRemoteMethod(this, "getSettings");
		markRemoteMethod(this, "setSettings");
		...
		return this.enqueueOperation(async () => {
			...
			if (projCache !== void 0 && typeof projCache.requireTable === "function") {
				try {
					...
					if (typeof projCache.rehome === "function") {
						for (const header of headers) {
							const record = projCache.requireTable().get(header.id);
							...
							report.projcacheRehomed.push(header.id);
						}
					}
				} catch (error) {
					this.ctx.logger.warn(`dsh-session-enhance: projection-cache cleanup failed: ${String(error)}`);
				}
			}
