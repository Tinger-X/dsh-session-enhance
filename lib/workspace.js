...
import { readSettings, sessionsRootFor, storagesRootFor, attachmentsRootFor, writeSettings } from "./settings.js";
...
import * as previewDomain from "./domains/preview.js";
...
var SessionEnhanceWorkspaceRegistry = class extends WorkspaceRegistry {
	...
	constructor(ctx) {
		...
		remoteMethod(this, "previewSession");
	...
	*/
	...
	/** `~/attachments` 目录（耐久图片附件存储根，尊重用户配置的 `.dsh` 家目录）。 */
	async attachmentsRoot() {
		const settings = await readSettings(this.settingsFile());
		return attachmentsRootFor(settings.homeDir);
	}
	...
	async previewSession(sessionId) {
		return previewDomain.previewSession(this, sessionId);
	}
