...
import { readSettings, storagesRootFor, writeSettings } from "./settings.js";
import { headerIdentity } from "./shared/identity.js";
import { remoteMethod } from "./shared/contracts/remote.js";
import { archivedBatchTargetSchema, settingsUpdateSchema, workspaceTargetSchema } from "./shared/contracts/schemas.js";
import { registerHostRemote } from "./shared/contracts/invocations.js";
import { getLiveSession, flushLiveSession, detachLiveSession, listLiveSessions, announceColdSessionRemoval } from "./upstream/sessions.js";
...
var SessionEnhanceWorkspaceRegistry = class extends WorkspaceRegistry {
	...
	*/
	...
	/** `~/storages` 目录（尊重用户配置的 `.dsh` 家目录；默认 `$DSH_HOME`/`~/.dsh`）。 */
	async storagesRoot() {
	...
	*/
	...
	async deleteSessionCore(sessionId) {
		if (!await this.sessionKnown(sessionId)) throw new SessionEnhanceUnknownSessionError(sessionId);
		const live = getLiveSession(this.ctx, sessionId);
	...
	*/
	async sweepStorageTraces(sessionId) {
		await sweepStorageRoot(await this.storagesRoot(), sessionId, this.ctx.logger);
	}
	/**
	...
	*/
	async verifyPhysicalDeletion(sessionId, transcriptDir) {
		...
		const storageTraces = await storageRootHasTraces(await this.storagesRoot(), sessionId);
	...
	*（fork 分支也携带它），而 fork 分支是独立的用户会话，绝不能被级联删除。 */
	async deleteDescendants(sessionId) {
		try {
			const descendants = [];
			for (const session of listLiveSessions(this.ctx)) {
				if (session.header.parentSession === sessionId && session.header.origin === "subagent") descendants.push(session.id);
			}
			for (const header of await this.ctx.get("sessionPersistence").list()) {
	...
	async cleanSpill(sessionId) {
		try {
			const root = getSpillRoot(this.ctx);
