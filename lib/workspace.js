...
/**
* dsh-session-manager 宿主侧会话管理。
*
* 在参考实现（@michengai/dsh-archive-manager）的归档语义之上，dsh-session-manager
* 增强了两处能力：
*
* 1. 物理删除保证：
...
const ARCHIVE_MANAGER_INVOCATIONS = [
	...
	{
		id: "dsh-session-manager#workspaceRegistry/verifyDeleted",
		service: "workspaceRegistry",
		namespace: "workspaceRegistry",
		...
		result: {
			mode: "strict",
			typeSymbol: "dsh-session-manager/types#DeletionVerification",
...
var ArchiveWorkspaceRegistry = class extends WorkspaceRegistry {
	...
	async archivedSessionMetadata() {
		...
		for (const sessionId of [...new Set(this.requireState().archivedSessionIds)]) {
			try {
				...
				this.ctx.logger.warn(`dsh-session-manager: could not read creation time for archived session "${sessionId}": ${String(error)}`);
			}
		}
		return { items };
	}
	...
	*/
	async verifyPhysicalDeletion(sessionId, transcriptDir) {
		...
		if (transcriptExists || storageTraces.length > 0) {
			...
			this.ctx.logger.warn(`dsh-session-manager: session "${sessionId}" was deleted but physical leftovers remain: ${leftovers}`);
		} else {
			this.ctx.logger.info(`dsh-session-manager: session "${sessionId}" physically deleted (transcript + storages/*.json verified clean)`);
		}
		...
		return this.enqueueOperation(async () => {
			...
			if (projCache !== void 0 && typeof projCache.requireTable === "function") {
				try {
					...
					this.ctx.logger.warn(`dsh-session-manager: projection-cache ghost cleanup failed: ${String(error)}`);
				}
			}
			// 5) 重建 header 索引：以当前物理列表为准（清除已消失会话的内存痕迹）。
			await this.replaceHeaderIndex(headers);
			this.ctx.logger.info(`dsh-session-manager: synced records against ${headers.length} physical session(s): +${report.workspaceAdded.length} accounted, -${report.archivedRemoved.length + report.workspaceRemoved.length + report.projcacheRemoved.length} ghosts`);
			return report;
		});
	}
	/**
	...
	* @throws {@link ArchiveUnknownSessionError} 会话未知时抛出；目标工作区未知或物理移动失败时抛 Error。
	*/
	async moveSession(sessionId, targetWorkspaceId) {
		return this.enqueueOperation(async () => {
				...
				if (typeof header?.cwd === "string" && header.cwd !== targetRecord.path) {
					...
					if (outcome.moved) {
						...
						try {
							...
							this.ctx.logger.warn(`dsh-session-manager: could not canonicalize new cwd "${targetRecord.path}" for moved session "${sessionId}": ${String(error)}`);
						}
						this.ctx.logger.info(`dsh-session-manager: physically moved transcript of "${sessionId}" from "${outcome.oldDir}" to "${outcome.newDir}"`);
					}
				}
			}
			...
			this.ctx.logger.info(`dsh-session-manager: moved session "${sessionId}" ${previousWorkspaceId === null ? "from ungrouped" : `from workspace "${previousWorkspaceId}"`} to ${targetWorkspaceId === null ? "ungrouped" : `workspace "${targetWorkspaceId}"`}`);
			return {
				sessionId,
				workspaceId: targetWorkspaceId ?? "",
				previousWorkspaceId: previousWorkspaceId ?? ""
	...
	*/
	async coldReuseKnown(id) {
		...
		try {
			...
			this.ctx.logger.warn(`dsh-session-manager: cold-reuse probe for "${id}" failed: ${String(error)}`);
			return false;
		}
	...
	*/
	...
	async publishColdSessionRemoval(sessionId, sessions) {
		...
		try {
			...
			this.ctx.logger.warn(`dsh-session-manager: could not publish removal for stored session "${sessionId}": ${String(error)}`);
		}
	}
	...
	* 带短间隔重试（Windows 句柄延迟释放），返回转录目录路径。 */
	async removeTranscriptDirectory(sessionId) {
		...
		this.ctx.logger.warn(`dsh-session-manager: ${detail}`);
	...
	*（fork 分支也携带它），而 fork 分支是独立的用户会话，绝不能被级联删除。 */
		...
		try {
			...
			for (const childId of descendants) {
				try {
					...
					this.ctx.logger.warn(`dsh-session-manager: cascade delete of subagent session "${childId}" (child of "${sessionId}") failed: ${String(error)}`);
				}
			}
		} catch (error) {
			this.ctx.logger.warn(`dsh-session-manager: descendant enumeration for deleted session "${sessionId}" failed: ${String(error)}`);
		}
	}
	...
	async cleanSpill(sessionId) {
		try {
			...
			this.ctx.logger.warn(`dsh-session-manager: spill cleanup for deleted session "${sessionId}" failed: ${String(error)}`);
		}
	}
};
//#endregion
