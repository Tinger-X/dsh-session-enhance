...
async function restoreSessionArtifact(file, oldDir, newDir, originalText, logger) {
	try {
		...
		logger.warn(`session-manager: rollback of physical session move failed: ${String(error)}`);
	}
}
/**
...
async function physicallyMoveSession(persistence, sessionId, header, newCwd, logger) {
	...
	try {
		...
		logger.warn(`session-manager: session "${sessionId}" physically moved, but persistence state refresh failed: ${String(error)}`);
	}
