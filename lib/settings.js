...
/**
 * 解析指定家目录下的 `sessions` 目录（会话转录根目录，与持久化后端的
 * `sessions/<projectKey(cwd)>/<id>/` 布局一致）。支持 `~` 前缀展开。
 * @param homeDir - 配置的家目录（如 `~/.dsh`）。
 * @returns `sessions` 目录绝对路径。
 */
function sessionsRootFor(homeDir) {
	return join(resolve(expandHomePath(homeDir)), "sessions");
}
/**
 * 解析指定家目录下的 `attachments` 目录（耐久图片附件的内容寻址存储根，
 * 布局为 `attachments/v1/objects/<sha256 前 2 位>/<sha256>`）。支持 `~` 前缀展开。
 * @param homeDir - 配置的家目录（如 `~/.dsh`）。
 * @returns `attachments` 目录绝对路径。
 */
function attachmentsRootFor(homeDir) {
	return join(resolve(expandHomePath(homeDir)), "attachments");
}
export { DEFAULT_SETTINGS, normalizeSettings, readSettings, sessionsRootFor, storagesRootFor, attachmentsRootFor, writeSettings };
